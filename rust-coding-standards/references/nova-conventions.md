...
### DOC-11: 註解 1–2 行,越簡單越好;表格與狀態圖除外

Leon:「code comment 除非是要畫表,或是狀態圖,其他請 agent 幫忙縮減成 1~2 行(描述越簡單簡潔越好)。」這條收緊了 Eden 原本的「預設 2–3 行」。判準仍用 Eden 那把尺:「記得寫 comments 是準備給人維運的時候讀的,想想修 Bug 的時候,你希望自己看到這邊留的是什麼,萬一當下用不了 AI 的話,你會期許這解釋要多長多精準。」一行寫不下的「為什麼」,先想能不能畫成表或圖(ASCII 佈局 / 時序圖 / 狀態圖),否則搬去 spec / runbook,歷史進 commit message / PR body。「怎麼做」看 code。doc comment 的 `# Errors` 之類小節也算在 1–2 行內,不另開段。agent 產註解慣性偏長(PR #105 ~370 行砍到 ~190 行仍嫌長;PR #153 整批再縮一輪,淨減 33 行),這是硬限制不是品味。
出處: PR #113 Eden review(`testsuite/on-target-stm32h5/memory.x` 7 行 → bank 圖 + 1 行,`e9c8c8c`,2026-09-03);PR #153 Leon review(1–2 行原則,`d765103`,2026-09-23)。
