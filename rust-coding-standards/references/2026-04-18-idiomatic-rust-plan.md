...
**狀態**: v0.7 拍板為定稿 (`<sync-reviewer>` 2026-04-29 核可,round 6 跳出迴圈判定成立,見 Appendix F)。Codex round 1-6 共 28 findings 全採納 + 三輪繁中品質驗證達標。v0.8 暫存進行中 — 第一個試跑 PR 自審累積 12 條實證心得待併入 Task 2.1 / Phase 5,索引見 §v0.8 暫存,全文證據留在工作追蹤系統同名章節。
...
- [ ] **Task 2.5**: **Binary-side idiom review gate** (`<binary-crate-paths>` 內部 PR 可實作標準)。
  - **範圍**: 涵蓋 MIN / MOD / CC 紅線之外的 Embassy / no_std 慣例 — heapless / const-generic buffer sizing、`static` lifetime 管理、feature gating 使用原則、panic handler / critical-section convention。**不**處理 Send / Sync justification (歸 Task 2.3)、ISR handoff pattern (歸 Task 3.5)、soundness (歸 Task 3.4)
  - **Acceptance**:
...
> **Illustrative (nrg-prototype PR α)**: PR α 拿 `<library-crate>::pgn::adlink::protocol` 新 module + `J1939Id::from_raw` / `to_raw` 跑 Task 2.1 + 2.2 + 2.3;audit 結果歸檔於 `references/int004/PR_ALPHA_API_GUIDELINES_AUDIT.md`。此例為 didactic reference,非通用模板規格。
...
- **PR β 之前完成 Phase 3**: PR β 會動更多 `<binary-crate-paths>` 硬體層 code,unsafe / concurrency verification 要先做完
...
## v0.8 暫存 — 試跑 PR 實證心得 (待併入清單)

**狀態**: 暫存進行中,等 Phase 2 Task 2.1 動工時依本章節的合併規則落地。

第一個通過新 pipeline 的 PR 自審期間累積 12 條實證心得,完整證據與條文落在工作追蹤系統的對應章節 (含每條的來源 commit hash、發現脈絡、條文)。本表是合併路線圖,讀者翻 plan 主文遇到 Task 2.1 章節時可從這裡反查「v0.8 哪些修訂會動這節」。**部分條目 (E/M/N) 的最終歸宿在本 plan 範圍外** (`CLAUDE.md` Workflow / `clean-code.md`),不算 plan 內部變動,但仍列在本表做暫存統一管理。

> **Illustrative (nrg-prototype PR α `ef363dba`)**: PR α (HEAD `ef363dba`,21 → 5 commit 收斂) 是 nrg-prototype 的試跑樣本;本段的 12 條心得來自 PR α 自審期間觀察。此例為 didactic reference,非通用模板規格。

表格左欄沿用工作追蹤系統各條原本的標題 (含原文的中英混搭),做為索引對齊用,Phase 2 Task 2.1 動工合併時再統一校稿;右欄為新寫的擬併入位置。**代號是穩定主鍵** — 兩邊以代號為對應依據,左欄摘要只是助讀者識別,落地前要跑漂移檢查 (合併規則步驟 1)。
...
1. **漂移檢查 (drift-check)** (落地前必跑): 核對代號集合 `{A, D, E, F, G, H, I, J, K, L, M, N}` 在 plan 檔本表與工作追蹤系統的對應章節**各出現一次**,且本表左欄摘要與工作追蹤系統各條 heading 一致。任一不符 → 先同步兩邊再進步驟 2。
2. **建立落地矩陣 (landing matrix)** (動工時): 在本章節下方加落地矩陣子段,逐條記錄落地進度:

   | 代號 | 來源 (工作追蹤系統段落) | 最終錨點 | 狀態 |
   |---|---|---|---|
   | A | 工作追蹤系統第 243 條 | `2026-04-18-idiomatic-rust-plan.md#task-2.1-...-rustdoc-跨-repo-隔離` | landed / pending |
   | ... | ... | ... | ... |
...
4. **原子性檢查 (atomicity gate)** (刪工作追蹤系統段前必查): 落地矩陣 12 列全部狀態 = landed + 對應錨點已存在於目的檔,才能刪除工作追蹤系統的對應章節;否則保留工作追蹤系統段,只把已落地的列在來源標記。
5. 全 12 列落地後,刪除工作追蹤系統該段,本章節改名為「v0.8 落地紀錄」,只留落地矩陣 (不留待辦表),plan 版本升 v0.8,頂端 status line 與 changelog 同步更新「v0.8 相對 v0.7 的主要變動」段。

**為何 v0.8 暫存拆兩處**: 工作追蹤系統是工作追蹤層,自然累積每日 PR 自審觀察;plan 檔是規格層,需要結構化的合併路線圖。暫存階段兩邊互補 (工作追蹤系統留全文證據避免 plan 檔膨脹,plan 檔留索引讓讀者反查)。Phase 2 Task 2.1 動工合併後,plan 內條目 (9 條) 收斂到 plan 主文,plan 外條目 (3 條) 收斂到 `CLAUDE.md` 對應段。
