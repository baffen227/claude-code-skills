...
> **VC / CI 媒介**: 本檔下文出現的 `<vc-ref-mechanism>` (PR review / Issue / comment ack 等媒介) 與 `<ci-platform>` (CI 執行媒介) 由 individual project 在 mapping doc §B 填具體值。本通用模板不指定具體工具。
...
- 163 列 hard-coded anchor 全數抽象化:(a) 80+ 列 → mapping §A 13 個 placeholder 中 9 個落地 substitution (其餘 `<workspace-pub-crates>` / `<detailed-reviewer-profile>` / `<ci-platform>` / `<release-branch>` 為 optional,個別 project 視需要引入);(b)(d) 46+ 列 → illustrative callout framing;(c) 35+ 列 → repo-neutral prose / `<vc-ref-mechanism>` / `<consensus-reviewer>` 等抽象化
...
- 改寫 Phase 3 啟動 gate 為 state-machine driven (綁 anchor spec §3.5.1 `<unsafe-targeted-test>` / `<unsafe-binary-side>` state machine,per spec §5);原 milestone-PR-specific gate 引用全部移入 illustrative callout
...
- [ ] **Task 2.1**: **Rust API Guidelines checklist** (`<library-crate>` public API gate)。從 `https://rust-lang.github.io/api-guidelines/checklist.html` 擷取全項,依本專案適用性分三級;**MUST / SHOULD 的啟用範圍綁定 API maturity tag** (見 §API maturity tag)。
  ...
  - **spec 覆蓋範圍 (v0.8 I)**: API doc 對應 spec 多 variant 場景時,明示 `# Covered` / `# Not covered` 或等價段 (`# Spec coverage`)。未明示的局部實作一律視為文件缺漏
  ...
  - **§testing 規範** (v0.8 J / L 落地):
    ...
    - **測試目的區分 (v0.8 J)**: test 寫進 codebase 前須區分 — independent regression guard (獨立 invariant 驗證) vs explanatory overlap (輔助理解特定 behaviour)。後者不算增加測試覆蓋,只是輔助說明
...
| E | 安全性修正的 commit message 必須含 caller 清查段 | landed (plan 外, fallback pending sync) | adlink-can `CLAUDE.md` §Coding Standards (Red Lines) 紅線 2 (per anchor spec §5.2 + §7.1);adlink-can 端待 maintainer 把 fallback inline 升級為 thin reference (per anchor spec §5.2) |
| F | Clippy 機械式 fix 不等於 rustdoc 乾淨 | landed | v1.0 Task 2.1 §審查流程「lint 與內容審視分層」(per spec §6.2) |
| G | 整體 file-by-file review 補 commit-by-commit 漏網 issue | landed | v1.0 Task 2.1 §審查流程「雙重通讀」(per spec §6.2) |
