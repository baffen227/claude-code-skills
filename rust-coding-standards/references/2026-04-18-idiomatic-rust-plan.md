...
| E | 安全性修正的 commit message 必須含 caller 清查段 | landed (plan 外, fallback pending sync) | adlink-can `CLAUDE.md` §Coding Standards (Red Lines) 紅線 2 (per anchor spec §5.2 + §7.1);adlink-can 端待 maintainer 將 fallback inline 升級為 thin reference (per anchor spec §5.2) |
