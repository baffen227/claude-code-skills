...
> **遷移備註 (2026-05-13)**: 本檔下文「Gitea issue 追蹤」「Gitea comment 拍板」「Gitea CI」等流程引用,從 2026-05-13 起改在 GitHub `github.com/fortuneBTBU/nrg-prototype` 走 (`gh` CLI / Issue / PR review)。舊 Gitea repo retired,留檔供歷史參考。Phase 1/2 落地時對應的 enforcement 媒介改為 GitHub Actions / `gh` 通知,實質內容 (lints / api-maturity tag / `cargo deny` 共識) 不變。

**目標**: 透過 Claude Code 達成 Rust 社群公認的最嚴格 Idiomatic Rust 實踐,覆蓋 library crate (`she_j1939`) 與 embedded binary crate (`she_bms_masterboard`) 兩側。
...
- **計畫把關** (Phase 5): 本計畫自身的 agent review + Harry 拍板。

**範圍**: 本專案涉及的 Rust crate — `she_j1939` (workspace library) 與 `she_bms_masterboard` (workspace-excluded binary)。不涵蓋 `nrg-prototype` 的其他 crate (Vincent 主導)。

**狀態**: v0.7 拍板為定稿 (Harry 2026-04-29 核可,round 6 跳出迴圈判定成立,見 Appendix F)。Codex round 1-6 共 28 findings 全採納 + 三輪繁中品質驗證達標。v0.8 暫存進行中 — PR α 自審累積 12 條實證心得待併入 Task 2.1 / Phase 5,索引見 §v0.8 暫存,全文證據留在 `tasks.md` 同名章節。
...
- Script spec 從 diff-based scan 改為**全庫 inventory scan**。原因: diff-based 只涵蓋新增 `pub` 行,漏掉「changed pub item」(簽名變、visibility promotion、doc 改但 `pub` 行未動)。全庫掃描 cost < 1 秒 (she_j1939 pub 項 < 100),換取 scope 完整涵蓋 new / changed / pre-existing 三類
...
- Trigger example path 統一 `crates/lib/` / `crates/bin/`
...
- `crates/lib/she_j1939/Cargo.toml` 有 `[lints] workspace = true`,繼承 workspace 設定
- `crates/bin/she_bms_masterboard` 排除在 workspace 外 (per-package-target 限制),workspace lints 不套用;需 crate 自己的 `[lints.clippy]` 跟上 (Task 1.1)
...
|------|------|--------------|---------------|
| `crates/lib/she_j1939/src/frame.rs:44` | `ExtendedId::new_unchecked` wrapper | Yes (conditional no_std) | miri targeted test (Task 3.1) |
| `crates/bin/she_bms_masterboard/src/heap_allocation.rs:25` | Heap 初始化 | No | unsafe-audit.md (Task 3.4) |
| `crates/bin/she_bms_masterboard/src/tasks/spawner.rs:32` | I2C1_ER interrupt handler decl | No | unsafe-audit.md + concurrency review (Task 3.5) |
| `crates/bin/she_bms_masterboard/src/tasks/spawner.rs:33` | `EXECUTOR_HIGH.on_interrupt()` | No | 同上 |
| `crates/bin/she_bms_masterboard/src/tasks/spawner.rs:38` | I2C1_EV interrupt handler decl | No | 同上 |
| `crates/bin/she_bms_masterboard/src/tasks/spawner.rs:39` | `EXECUTOR_MEDIUM.on_interrupt()` | No | 同上 |

Host-testable 只有 1/6 (`frame.rs:44`)。其餘 5 處 Phase 3 用手動 audit + Embassy concurrency review。
...
| Binary-side idiom (heapless / feature gating / panic handler) | **Task 2.5 binary-idioms-checklist** | — | 對 `crates/bin/she_bms_masterboard` 內部 PR 的可實作標準;unsafe soundness 仍由 Task 3.4 主導 |
...
| Tag | 定義 | 範例 | MUST 啟用範圍 |
|-----|------|------|---------------|
| `stable public` | 已確認會對外發布或跨組消費,semver 承諾已 establish | 未來 `she_j1939` 若 publish 到 crates.io 或 ADLINK 直接 link | 完整 API Guidelines MUST + SHOULD (含 sealed trait / `non_exhaustive` / `From` / `TryFrom` 對稱性) |
| `workspace-internal pub` | `pub` 但消費者只在 workspace 內 (現 `she_j1939` 大部分 API 屬此類) | `J1939Id::from_raw` / `to_raw` / `pgn::adlink::protocol::*` | API Guidelines MUST (命名、Debug、`# Errors` / `# Panics` / `# Safety`、`Clone` / `Default` 立場);future-proof 項目 (sealed trait / `non_exhaustive` / smart-pointer conversion) 降為 SHOULD,待 tag 升級到 `stable public` 才 MUST |
| `temporary pub` | 短期過渡 (為了測試或 migration 暫時 `pub`),commit 必須附刪除計畫 | PR α 過渡期間的 `pub(crate)` 擴展成 `pub` | API Guidelines 全降為 MAY;必須有 TODO comment + Gitea issue 追蹤 deprecation |
...
- **Executable enforcement** (Task 2.1 acceptance 必跑): 由 `scripts/check-api-maturity-tags.sh` 執行,script spec 如下 (實作由 Task 1.6 同批產出)。**採全庫 inventory scan,不用 diff-based** — v0.5 已驗證 diff-based 無法涵蓋「changed pub item」(簽名變、visibility promotion `pub(crate)` → `pub`、doc 改但 `pub` 行未動) 的情境;v0.4 的 awk 實作也在 round 4 已驗證為錯誤 (hunk-wide flag 誤判 + 未處理 EOF),兩者皆**移除不用**:
  - **Full inventory scan**: script 對 `crates/lib/**/*.rs` 所有 source file 內**所有 crate-externally-visible** `pub` item 逐項掃描 (she_j1939 pub 項 < 100,全掃成本可忽略,執行 < 1 秒)。覆蓋種類**不限於但至少包含**:
...
- **Temporary pub deprecation**: 每個 `temporary pub` item 必須在 Gitea 開對應 issue,issue number 寫在 rustdoc 的 `<!-- api-maturity-deprecation: #NNNN -->` 第二個 marker
...
| ISR 和 task 共享 `UnsafeCell<T>`,無 atomic 或 critical-section 包裹 | **Task 3.5** (跨 context Send / Sync correctness) | Task 3.4 (`// Safety:` in-code 註解必須 cite 3.5 finding ID) |
...
| `she_j1939::J1939Id::from_raw` 是否有 `# Errors` + `impl Debug` + `#[must_use]` | **Task 2.1** (API Guidelines MUST) | — |
...
| `she_bms_masterboard` 的 `static FOO: Mutex<...> = Mutex::new(...)` buffer 大小用 magic number | **Task 2.5** (binary-idioms: heapless / const-generic buffer sizing) | — |
...
### Phase 1 — 工具基線 (1A 本週;1B 視 Vincent / Harry 決策時間)

Target: 把 pipeline 第 3-4 步完整接入工作流程,讓 commit 前 checklist 不再掛漏。拆成 1A / 1B 兩組,避免外部決策拖住可獨立落地部分。

**1A — 可獨立落地 (不需跨人共識)**:

- [ ] **Task 1.1**: 確認 `crates/bin/she_bms_masterboard/Cargo.toml` 的 `[lints.clippy]` 跟 workspace 對齊 (因為排除在 workspace 外,workspace lints 不自動套用)。若沒對齊,補上 `nursery = deny` + `pedantic = deny`。
  - **Acceptance** (必須三項全過才算完成):
    1. `cargo clippy --manifest-path crates/bin/she_bms_masterboard/Cargo.toml --all-targets -- -D warnings` 退出碼 0 (注意: 因為 `she_bms_masterboard` 是 workspace-excluded,**不可**用 `-p she_bms_masterboard`,必須 `--manifest-path`)
    2. 任何 `#[allow(...)]` 必須附註單行 justification (grep `#\[allow` 每個 hit 都有 `// reason:` 下一行)
    3. `Cargo.toml` diff 加入 PR,Vincent 在 Gitea 有留 `ack` 或 emoji ack
- [ ] **Task 1.2**: `cargo doc -D rustdoc::broken_intra_doc_links -D rustdoc::invalid_rust_codeblocks` 落到 pre-commit 腳本。
  - **Acceptance**:
    ...
    2. 測試 fixture (必須在 `cargo doc` 偵測範圍內,不可用 `docs/_test_broken.md` 這種非 rustdoc 檔): 在 `crates/lib/she_j1939/src/lib.rs` 或任一 `pub` item 的 rustdoc 內加一個刻意 broken 的 intra-doc link (例如 `//! [`NonexistentType`]`),執行 `cargo doc` 應報 error,hook 應擋下 `git commit`;測試完成後 revert fixture
...
- [ ] **Task 1.6**: 產出 `scripts/rust-quality-check.sh`。介面拆成兩段避免工具語義錯配:
  ...
  - `she_bms_masterboard` 是 workspace-excluded,只吃 `--crate` 不吃 `--workspace-policy`
  - **Acceptance**:
    1. 腳本存在且 `bash -n scripts/rust-quality-check.sh` 語法檢查過
    2. 4 種情境實測: `--crate she_j1939` 成功 / `--crate she_bms_masterboard` 成功 / `--workspace-policy` 成功 / `--crate she_bms_masterboard --workspace-policy` 必須退出碼 2 + stderr 印錯誤訊息
...
**1B — 需 Vincent / Harry 共識 (不擋 1A 落地)**:

- [ ] **Task 1.3**: 評估 `cargo audit` 從月度升為 PR 級。執行成本約 5 秒;本專案 embedded 相依套件不多,但都關鍵 (`critical-section` / `cortex-m` / `embassy-*` 都是核心 crate),CVE 一出就要追。
  - **Acceptance**:
    1. Harry + Vincent 至少一方在 Gitea comment 留下拍板 (PR 級 / 月度) 並標示理由
    2. CLAUDE.md Rust verification checklist 對應段落更新,引用該 comment URL
    3. 若升 PR 級,`rust-quality-check.sh --workspace-policy` 涵蓋該 step
- [ ] **Task 1.4**: 評估 `cargo deny check` (workspace 層,license / duplicate / banned deps)。需跟 Vincent 共識。
  - **Acceptance**:
    1. Vincent 合入 `nrg-prototype/Cargo.toml` 的 `[workspace.metadata.deny]` 段落,或 Gitea comment 明確同意 / 拒絕
...
- Vincent 共識 `cargo deny` workspace 設定 (1B)
...
- [ ] **Task 2.1**: **Rust API Guidelines checklist** (`she_j1939` public API gate)。從 `https://rust-lang.github.io/api-guidelines/checklist.html` 擷取全項,依本專案適用性分三級;**MUST / SHOULD 的啟用範圍綁定 API maturity tag** (見 §API maturity tag)。
  - **觸發範圍**: library crate (`crates/lib/she_j1939`) 的 new / changed `pub` item。Binary-only PR 不觸發此層。
  ...
  - **Acceptance** (必須五項全過):
    ...
    3. `## 範例表`下,每個三級項**至少一個** `she_j1939` pass 範例 + 一個 fail 範例 (或標 `[pending: 待 Task 2.6 試跑累積]`,不得空白)
    4. `## Trigger 規則` 段有 diff pattern 的 shell one-liner 範例 (例如 `git diff --name-only ... | grep '^crates/lib/she_j1939/src/' && git diff | grep -qE '^\+.*pub '`)
...
- [ ] **Task 2.2**: **Error taxonomy policy**。定義本專案的 error 設計策略。
  - **範圍**: `she_j1939` public errors / `she_bms_masterboard` internal errors 各自的 policy
  ...
  - **Acceptance**:
    ...
    2. 每個 section 至少一個 `she_j1939` 或 `she_bms_masterboard` 現存或預定範例 (不接受純敘述)
...
- [ ] **Task 2.3**: **Ownership & conversion idioms**。定義公開 API 的 ownership 慣例與 auto-trait justification。此為 Send / Sync justification 的 canonical home (見 §Ownership concern canonical home)。
  ...
  - **Acceptance**:
    ...
    3. 每個 section 至少 3 個 `she_j1939` 範例,範例的狀態用 `pass` / `pending` / `fail` 明確標記
- [ ] **Task 2.4**: **Embassy async checklist** — **local `.await` 點的 state-machine obligations**。
  ...
  - **Acceptance**:
    ...
    2. `## 每個 .await 點的 local obligations` 至少 5 個 `crates/bin/she_bms_masterboard` 現存 `.await` 點 (例如 `ti_001_can_comm::wait_with_demux`) 的逐點對照
    3. `## Trigger 規則` 段給 diff pattern shell one-liner (例如 `git diff | grep -qE '^\+.*\.await|^\+.*async fn|^\+.*select!'`),且必須 path 限縮 `crates/bin/`
- [ ] **Task 2.5**: **Binary-side idiom review gate** (`crates/bin/she_bms_masterboard` 內部 PR 可實作標準)。
  - **範圍**: 涵蓋 MIN / MOD / CC 紅線之外的 Embassy / no_std 慣例 — heapless / const-generic buffer sizing、`static` lifetime 管理、feature gating (例如 `hse_8mhz`) 使用原則、panic handler / critical-section convention。**不**處理 Send / Sync justification (歸 Task 2.3)、ISR handoff pattern (歸 Task 3.5)、soundness (歸 Task 3.4)
  - **Acceptance**:
    1. 產出 `docs/coding-standards/binary-idioms-checklist.md`,section: `## heapless / const-generic sizing` / `## static lifetime` / `## feature gating` / `## panic handler / critical-section` / `## 與 Task 2.3 / 3.4 / 3.5 redirect`
    2. 每個 section 至少 2 個 `crates/bin/she_bms_masterboard` 範例
    3. `## redirect` 段列出本 task **不處理**的 3 類 concern (Send / Sync、ISR handoff、soundness),各給 owning task 連結
- [ ] **Task 2.6**: **PR α 試跑 audit**。拿 PR α (`she_j1939::pgn::adlink::protocol` 新 module + `J1939Id::from_raw` / `to_raw`) 跑一次 Task 2.1 + 2.2 + 2.3 checklist,作為自用試跑。
  - **Acceptance**:
    1. 產出 `references/int004/PR_ALPHA_API_GUIDELINES_AUDIT.md` (一次性,PR α merge 後歸檔)
...
- `references/int004/PR_ALPHA_API_GUIDELINES_AUDIT.md` (Task 2.6,一次性)
...
2. Agent B: 試跑 PR α 結果 (Task 2.6)

---

### Phase 3 — Unsafe / concurrency verification (PR β 前)

Target: pipeline 第 5 步的完整建置。本專案因為 embedded + no_std,不能照搬 std 專案慣例。
...
- [ ] **Task 3.1**: **miri targeted tests for `she_j1939`**。
  - **指定 target tests**: Phase 3 要明列具體 test function 名稱跑到 `crates/lib/she_j1939/src/frame.rs:44` `ExtendedId::new_unchecked`。候選 test (待實做時確認命名): `frame::tests::new_extended_with_max_id_constructs_ok` / `frame::tests::new_extended_with_boundary_ids_round_trip`。必須有 test 實際實例化 `J1939Frame::new(...)` 觸及 unsafe block,否則 `cargo miri test` 等於 no-op。
  - **Acceptance**:
    ...
    3. `## 執行命令`給完整 `cargo +nightly miri test -p she_j1939 --test <name> -- <filter>` 範例
...
- [ ] **Task 3.2**: **miri 工具比較表**。
  ...
  - **Acceptance**:
    ...
    2. 至少對 `frame.rs:44` targeted test 跑過 baseline + `-Zmiri-tree-borrows` 兩組,結果 (pass / UB + 原因) 記錄在表下方
- [ ] **Task 3.3**: **Concurrency verification meta-policy** (v0.3 收窄為 meta-policy)。
  ...
  - **結論修正**: loom 以當前 API 直接用於 Embassy 不適用 (loom 目標是 `std::sync::{Arc, Mutex, RwLock}` 的 memory ordering,Embassy `Signal` / `Channel` / `Mutex` 是 cooperative executor 內部 primitive,不走 `std::sync`);但「Embassy = 單 executor = 無並發驗證」的推理錯誤。Embassy 仍有 interrupt ↔ task interleaving、atomics / critical-section 使用假設、wakeup ordering、executor affinity、`Send` / `Sync` 跨任務邊界等正確性問題,由 Task 3.5 接手
  - **替代管控機制**:
...
- [ ] **Task 3.5**: **Embassy concurrency review checklist** (跨 context interleaving / ordering / affinity 的 canonical home)。
  - **範圍 (v0.4 釐清)**: ISR ↔ task interleaving、wakeup ordering 依賴、atomics / critical-section 使用假設、**`Send` / `Sync` 跨 context 的 runtime correctness** (實際 interleaving 下是否成立;**不**涵蓋 public type 的 Send / Sync justification doc — 見 Task 2.3)、executor affinity (哪個 executor 跑哪個 task)。**個別規則的 canonical home**,Task 2.4 / 3.3 / 3.4 都 link 到此,不複述
  - **Acceptance**:
    1. 產出 `docs/coding-standards/embassy-concurrency-review.md`,section: `## ISR ↔ task interleaving` / `## wakeup ordering` / `## atomics / critical-section` / `## Send / Sync 跨邊界` / `## executor affinity` / `## 與 Task 2.4 / 3.3 / 3.4 redirect`
    2. 每個 section 至少一個 `crates/bin/she_bms_masterboard` pass 範例 + 一個 anti-pattern fail 範例
    3. 與 `unsafe-audit.md` 互引 (每處 interrupt handler unsafe 必有對應 concurrency review 項目)

**Miri 觸發頻率**:

- **PR 級**: 觸發於 `crates/lib/she_j1939` unsafe 相關 test 或 unsafe code 變動時 (diff pattern: `crates/lib/she_j1939/src/frame.rs` 變動或 test 檔變動且檔內有 unsafe 相關 fn)。若 nightly toolchain 破損,跳過並在 `tasks.md` 記錄,不擋主流程
...
Target: reviewer (Harry + Vincent) 審查能力養成。規則引擎 / checklist 是「能辨識」,深度閱讀是「能設計」。
...
- [ ] **Task 4.2**: 跟 Vincent 約每月一對一同步進度 (或以 Gitea 留言非同步更新)
...
- [x] **Checkpoint 5.1** (必要): Codex adversarial review。CLAUDE.md 設計文件紅線要求,本檔名符合 `*-plan.md` pattern。focus 建議: Phase 分期、idiomatic Rust 覆蓋面、Phase 3 推理。
  ...
  - **v0.7 → 定稿**: **closed (Harry 2026-04-29 核可跳出迴圈判定)** — Claude 依 CLAUDE.md `feedback_consensus_loop_escalation` 判跳出迴圈 (round 3-6 連 4 輪皆在同一 spec 抓 edge case,層次從結構 → 實作 → scope → enumeration 收斂到細節),Harry 2026-04-29 接受此判定,v0.7 為定稿
- [ ] ~~**Checkpoint 5.2** (非必要): Agent B sanity review。focus: 每個 Task 的交付物是否具體可驗證~~ — **skipped (Harry 2026-04-29 選 C 直接拍板 + 起 v0.8 暫存,跳過 5.2)**
- [x] **Checkpoint 5.3** (必要): Harry 人工拍板 — **done (2026-04-29)**
...
- **Q1**: `cargo audit` 升為 PR 級 vs 月度? (Phase 1 Task 1.3)
  - 成本約 5 秒、訊號中
  - embedded 相依套件不多 (`cortex-m` / `embassy-*` / `critical-section`),但核心 crate 一出 CVE 就要追
  - 建議升 PR 級,等 Harry / Vincent 拍板
- **Q2**: `miri` 在本專案實際覆蓋度? (Phase 3 Task 3.1)
  - 6 處 unsafe 只 1 處 host-testable (`frame.rs:44`)
  - 但這 1 處正是 `new_unchecked`,miri 強項 — 值得 PR 級跑 (條件觸發 + 月度 drift detection)
...
| 7 | — | — | **Claude 判跳出迴圈** (round 3-6 同一 spec 4 輪 edge-case chain);若 Harry 明示需 round 7 再跑 |
...
| 7 | Phase 2 試跑 (PR α audit) | Agent B | 跑完 PR α audit 後 |
...
## v0.8 暫存 — PR α 實證心得 (待併入清單)

**狀態**: 暫存進行中,等 Phase 2 Task 2.1 動工時依本章節的合併規則落地。

PR α (HEAD `ef363dba`,21 → 5 commit 收斂) 自審期間累積 12 條實證心得,完整證據與條文落在 `tasks.md` §v0.8 待併入清單 (含每條的來源 commit hash、發現脈絡、條文)。本表是合併路線圖,讀者翻 plan 主文遇到 Task 2.1 章節時可從這裡反查「v0.8 哪些修訂會動這節」。**部分條目 (E/M/N) 的最終歸宿在本 plan 範圍外** (`CLAUDE.md` Workflow / `clean-code.md`),不算 plan 內部變動,但仍列在本表做暫存統一管理。

表格左欄沿用 `tasks.md` 各條原本的標題 (含原文的中英混搭),做為索引對齊用,Phase 2 Task 2.1 動工合併時再統一校稿;右欄為新寫的擬併入位置。**代號是穩定主鍵** — 兩邊以代號為對應依據,左欄摘要只是助讀者識別,落地前要跑漂移檢查 (合併規則步驟 1)。

| 代號 | 一句摘要 | 擬併入位置 |
|---|---|---|
| A | Library crate rustdoc 不可引用消費者看不見的設計文件 | Task 2.1 §SHOULD (新增 rustdoc 跨 repo 隔離條) |
| D | Helper naming 看 module 既有 identifier 脈絡 | Task 2.1 §MUST (C-CASE / C-CONV) — 命名反例 |
| E | 安全性修正的 commit message 必須含 caller 清查段 | **plan 外** → `CLAUDE.md` §Coding Standards (Red Lines) 新增 commit 訊息規範條 (commit 訊息規範是跨任務工作流,不是 idiomatic Rust 本體) |
| F | Clippy 機械式 fix 不等於 rustdoc 乾淨 | Task 2.1 §審查流程 (新增 lint 與內容審視分層條) |
| G | Holistic file-by-file review 補 commit-by-commit 漏網 issue | Task 2.1 §審查流程 (新增雙重通讀審查條) |
| H | `u32::from_le_bytes` 是表達 byte-order 意圖的 idiomatic 寫法 | Task 2.1 §SHOULD (新增位元組順序轉換條) |
| I | Spec 多變體時 doc 要明說 covered / not covered | Task 2.1 §SHOULD (新增 spec 覆蓋範圍條) |
| J | Divergence test 是 explanatory overlap,不是 independent regression guard | Task 2.1 §testing 規範 (新增測試目的區分條) |
| K | Dual sub-agent review 30% overlap + 70% 互補 | Task 2.1 §審查流程 (新增雙視角審查條) |
| L | MIN-1 紅線在 test 同樣適用 | Task 2.1 §testing 規範 (引 MIN-1 至 test 範疇) |
| M | 逐筆搬比 git rebase --squash 對 PR history 收斂更乾淨 | **plan 外** → `CLAUDE.md` §Workflow Preferences 新增 PR history 收斂手法條 |
| N | PR description for detailed reviewer 必須含 commit walkthrough + review order | **plan 外** → `CLAUDE.md` §Workflow Preferences 新增 PR description 結構條 |

**合併規則** (Phase 2 Task 2.1 啟動時執行,順序不可跳):

1. **漂移檢查 (drift-check)** (落地前必跑): 核對代號集合 `{A, D, E, F, G, H, I, J, K, L, M, N}` 在 plan 檔本表與 `tasks.md` §v0.8 待併入清單**各出現一次**,且本表左欄摘要與 `tasks.md` 各條 heading 一致。任一不符 → 先同步兩邊再進步驟 2。
2. **建立落地矩陣 (landing matrix)** (動工時): 在本章節下方加落地矩陣子段,逐條記錄落地進度:

   | 代號 | 來源 (`tasks.md` 段落) | 最終錨點 | 狀態 |
   |---|---|---|---|
   | A | `tasks.md:243` | `2026-04-18-idiomatic-rust-plan.md#task-2.1-...-rustdoc-跨-repo-隔離` | landed / pending |
   | ... | ... | ... | ... |
3. 對 plan 內條目 (A/D/F/G/H/I/J/K/L,共 9 條): 把條文與證據摘要落地進對應 Task 2.1 子段;對 plan 外條目 (E/M/N,共 3 條): 落地進對應外部檔 (`CLAUDE.md` 等),plan 端只在落地矩陣記錨點。
4. **原子性檢查 (atomicity gate)** (刪 `tasks.md` 段前必查): 落地矩陣 12 列全部狀態 = landed + 對應錨點已存在於目的檔,才能刪除 `tasks.md` §v0.8 待併入清單;否則保留 `tasks.md` 段,只把已落地的列在來源標記。
5. 全 12 列落地後,刪除 `tasks.md` 該段,本章節改名為「v0.8 落地紀錄」,只留落地矩陣 (不留待辦表),plan 版本升 v0.8,頂端 status line 與 changelog 同步更新「v0.8 相對 v0.7 的主要變動」段。

**為何 v0.8 暫存拆兩處**: `tasks.md` 是工作追蹤層,自然累積每日 PR α 的自審觀察;plan 檔是規格層,需要結構化的合併路線圖。暫存階段兩邊互補 (`tasks.md` 留全文證據避免 plan 檔膨脹,plan 檔留索引讓讀者反查)。Phase 2 Task 2.1 動工合併後,plan 內條目 (9 條) 收斂到 plan 主文,plan 外條目 (3 條) 收斂到 `CLAUDE.md` 對應段。
...
|---|-----------------|-------|--------------|------|---------------|
| P1-1 | `she_bms_masterboard` 無 operational idiom review gate | P1 | Phase 2 Task 2.5 (binary-idioms-checklist) | fixed | 無 — v0.3 以 ownership concern canonical home 進一步釐清 Task 2.5 不處理 unsafe / ISR / Send-Sync |
...
| R2-P2-4 | MIN-1 vs future-proof 建議長期拉扯;缺 API maturity tag | P2 | §API maturity tag 新增 (3 tag);Task 2.1 MUST / SHOULD 綁 tag | fixed | 每個 `pub` item 的 tag 選擇需 reviewer 判斷,預設 `workspace-internal pub` 並可升級;待 Task 2.6 試跑 PR α 時驗證 |
...
| R3-3 | Task 1.2 fixture (`docs/_test_broken.md`) 在 `cargo doc` 偵測範圍外,驗不到 hook | P2 | Task 1.2 Acceptance 第 2 條改為「在 crate rustdoc 內加 broken intra-doc link」(`crates/lib/she_j1939/src/lib.rs` 或任一 `pub` item) | fixed | 無 |
| R3-4 | Task 1.1 用 `-p she_bms_masterboard` 與 workspace-excluded 矛盾 | P2 | Task 1.1 Acceptance 第 1 條改為 `--manifest-path crates/bin/she_bms_masterboard/Cargo.toml`,附註說明 | fixed | 無 |
...
|---|-----------------|-------|--------------|------|---------------|
| R5-1 | Script spec 用 diff-based 掃描,但 plan 規則 scope 說「new / changed pub item」;「changed pub item」(簽名變、visibility promotion `pub(crate)` → `pub`、doc 改但 `pub` 行未動) 無法檢測 | structural | §API maturity tag 的「Executable enforcement」段改為**全庫 inventory scan**;每次 pre-commit / CI 掃 `crates/lib/**/*.rs` 全部現存 pub item,不依 diff。Cost < 1 秒 | fixed | 無 |
...
### Escalation decision rationale (Claude 判定,等 Harry 核可)
...
若 Harry 核可,v0.7 即為 Phase 5.1 final draft;否則 Harry 可推翻此判定,繼續 round 7。
| R2-P2-6 | Trigger example path 不一致 (`she_j1939/src/...` vs `crates/lib/she_j1939/src/...`) | P2 | 全檔統一 `crates/lib/` / `crates/bin/` path | fixed | 無 |
