...
#
# 用途: 把 stdin 的筆記正文寫到 ~/Obsidian/Notes/<title>.md (直落永久區)
#
# 用法:
#   write-draft.sh "<陳述句標題>" < note-body.md
#   echo "<note body>" | write-draft.sh "<陳述句標題>"
#
# 2026-08-08 Obsidian 重定位後: 不再經 Notes/inbox/ 暫存、檔名不帶日期前綴
# (created 日期在 frontmatter)。Obsidian CLI 整合預留 (見 maybe_use_cli)。

set -euo pipefail

if [ "$#" -ne 1 ]; then
    echo "用法: $0 <標題>" >&2
    echo "  從 stdin 讀筆記正文，寫到 ~/Obsidian/Notes/<標題>.md" >&2
    exit 2
fi
...
COUNTER=2
while [ -e "$TARGET" ]; do
    TARGET="$NOTES_DIR/${SAFE_TITLE}-${COUNTER}.md"
    COUNTER=$((COUNTER + 1))
done
